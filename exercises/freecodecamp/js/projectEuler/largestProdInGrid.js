function largestGridProduct(arr) {
    ...
    function diagonalRihgth(arr){
        for(let k = 0; k <= arr.length -4; k++){
            for (let m = 0; m <= arr.length -4; m++){
                let prod = 1;
                for (let i = k, j = m; i < 4+k, j < 4+m; i++, j++){
                    prod *= arr[i][j];
                }
                // console.log(prod);
                if (prod > res) res = prod;
    ...
    function diagonalLeft(arr){
        for(let k = 0; k <= arr.length-4; k++){
            for (let m = arr.length-1; m >= 4-(arr.length-4); m--){
                let prod = 1;
                for (let i = k, j = m; i < 4+k, j > m-4; i++, j--){
                    prod *= arr[i][j];
                }
                // console.log(prod);
                if (prod > res) res = prod;
