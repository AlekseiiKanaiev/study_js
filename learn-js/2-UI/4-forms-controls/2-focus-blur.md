...
В работе с событиями фокусировки есть важные особенности.
...
Your email: <input type = "email" id = "input">
<input type = "text" style = "width: 280px" placeholder = "enter incorrecrt email and click here">

<script>
    input.onblur = () => {
        if (!input.value.includes('@')){
            // не email -> показать ошибку
