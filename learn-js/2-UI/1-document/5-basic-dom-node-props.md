...
Pазница между `tagName` и `nodeName`:
...
<body>
    ...
    <script>
        // для комментария
        alert(document.body.firstChild.tagName); // indefined (не элемент)
...
<body>
    ...
    <script>
        ...
        document.body.innerHTML = 'NEW BODY'; //  заменяем содержимое
    </script>
</body>
```
...
<body>
    <script>
        document.body.innerHTML = '<b>test'; // забыли закрыть тег
...
войство `outerHTML` содержит HTML элемента целиком. Это как `innerHTML` плюс сам элемент.
...
<script>
    let name = prompt("Enter your name", "<b>Винни-пух!</b>");
