...
<div class = "alert">
    <strong>Hello to all!</strong>
    Вы прочитали важное сообщение.
</div>
```
...
let div = document.createElement('div');
div.className = 'alert';
div.innerHTML = '<strong>Hello to all!</strong> Вы прочитали важное сообщение.';
```
...
<script>
    ...
    div.insertAdjacentHTML('afterend', '<p>Goodbye</p>');
</script>
```
...
<script>
    document.body.insertAdjacentHTML('afterbegin', '<div class = "alert"><strong>Hello to all!</strong> Вы прочитали важное сообщение.</div>');
</script>
```
...
<script>
    let div = document.createElement('div');
    div.innerHTML = '<p>Hello to all!</p>';
    div.className = 'alert';
    document.body.append(div)
...
<div class = "alert" id = "div"><strong>Hello to all!</strong></div>

<script>
    let div2 = div.cloneNode(true); // клонировать сообщение
    div2.querySelector('strong').innerHTML = 'Boodbye!'; // изменить клонированный элемент
...
Мы упоминаем `DocumentFragment` в основном потому, что он используется в некоторых других областях, например, для элемента [template](info:template-element).
