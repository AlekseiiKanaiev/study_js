...
<body>
  <script>
    ...
    // сейчас добавим единицу измерения (px)
...
`getComputedStyle` был создан для получения вычисленных значений, но оказалось, что окончательные значения гораздо удобнее, и стандарт изменился.
...
### "Стили, примяемые к посещённым `:visited` ссылкам, скрываются!"
