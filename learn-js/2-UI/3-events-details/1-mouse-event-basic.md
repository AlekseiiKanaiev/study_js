...
```html 
<button id = "button">Press Alt+Shift+Click on me</button>
...
Наведите курсор мыши на поле ввода, чтобы увидеть `clientX/clientY` и `pageX/pageY`:
...
<div>
    <input onmousemove="logMouse(event)"
    value="Hover me"
    style="margin: 100px; height: 40px">
</div>
...
Есть несколько способов запретить выделение.
...
```html
<div oncope = "alert('Copy is forbiden!'); return false">
