...
```html
<input id = "elem" type = "button" value = "Push it">
<script>
...
```html
<input id = "elem" type = "button" onclick = "alert('Yes')" value = "Push it!">
<script>
    elem.onclick = function() {
        // перезапишет существующий обработчик
        alert('This one'); // выведется только это
    }
</script>
```
...
Как видно из примера выше, можно одновременно назначать обработчики и через DOM-свойство и через `addEventListener`. В примере выше сработаю все 3 обработчика. Однако, во избежание путаницы, рекомендуется выбрать один способ.
...
<input type="button" id="elem" onclick="this.class.toggle('wide')" value="Push it">

<script>
    elem.ontransitioned = () => alert('DOM property'); // не сработает
    elem.addEventListener('transitioned', () => alert('addEventListener')); // сработает по окончании анимации  
</script>
```
...
```html run
<input type = "button" id = "elem" value = "Push it">
<script>
...
<script>
  ...
  let menu = new Menu();
  elem.addEventListener('mousedown', munu);
  elem.addEventListener('mouseup', menu);
</script>
```
