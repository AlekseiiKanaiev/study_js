...
```html
<h1 id = "elem">Hello form code!</h1>

<script>
    document.addEventListener('hello', (event) => { // (1)
        alert("hello form " + event.target.tagName); // hello from h1
    });
...
Порядок вывоа будет: 1 -> вложенное событие -> 2.
