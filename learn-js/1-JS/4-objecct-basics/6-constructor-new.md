...
Когда функция вызывается как `new User(...)`, происходит сдедующее:
...
Т.е. результат вызова `new  User("Вася")` - это тот же объект, что и:
...
Для создания сложных объектов есть и более "продвинутый" синтаксис - [классы](info:classes).
...
JavaScript предоставляет функции-конструкторы для множества встроенных объектов языка: например, `Date`, `Set` и других.
