...
Promise.all([
    ...
    new Promise(res => setTimeout(() => res(1), 2000)), // 2
    new Promise(res => setTimeout(() => res(1), 1000)), // 3
])
...
const requests = urls.map(url => frtch(url));
...
`Promise.all` ничего не делает для их отмены, так как в промисах вообще нет концепии "отмены".
